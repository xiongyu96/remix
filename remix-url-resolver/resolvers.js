...
module.exports = class CompilerImports {
  ...
  handlers () {
    return [
      { type: 'github', match: (url) => { return /^(https?:\/\/)?(www.)?github.com\/([^/]*\/[^/]*)\/(.*)/.exec(url) }, handler: (match, cb) => { this.handleGithubCall(match[3], match[4], cb) } },
      { type: 'http', match: (url) => { return /^(http?:\/\/?(.*))$/.exec(url) }, handler: (match, cb) => { this.handleHttpCall(match[1], match[2], cb) } },
      { type: 'https', match: (url) => { return /^(https?:\/\/?(.*))$/.exec(url) }, handler: (match, cb) => { this.handleHttpCall(match[1], match[2], cb) } },
      { type: 'swarm', match: (url) => { return /^(bzz-raw?:\/\/?(.*))$/.exec(url) }, handler: (match, cb) => { this.handleSwarmImport(match[1], match[2], cb) } },
      { type: 'ipfs', match: (url) => { return /^(ipfs:\/\/?.+)/.exec(url) }, handler: (match, cb) => { this.handleIPFS(match[1], cb) } }
    ]
  }
  ...
  resolve (url, loadingCb, customHandlers, cb) {
    var self = this
    ...
    handler = [...handler, customHandlers]
    var found = false
    handlers.forEach(function (handler) {
      ...
      if (match) {
        ...
        handler.handler(match, function (err, content, cleanUrl) {
          ...
          // fileProvider[github].name = content
